...
def main(argv=None) -> int:
    ...
    if len(missing):
        ...
        # The median passing is NOT the test. Last run it passed - $3,000
        # against $4,000 - while an $8,500 quarterback sat in the unmatched
        # list because of a full stop in his initials. A summary statistic
        # that survives a broken join is a summary statistic that cannot
        # detect one, so the tail gets its own explicit check.
        cutoff = merged["salary"].quantile(0.90)
        pricey = missing[missing["salary"] >= cutoff]
        print(f"  top-decile price is ${cutoff:,.0f}; {len(pricey)} unmatched "
              f"players sit at or above it")
        if len(pricey):
            print("  -> these are NOT bench players. Every one of them is a "
                  "name the join failed on, and each is worth reading:")
            for r in pricey.nlargest(10, "salary").itertuples(index=False):
                print(f"       ${r.salary:>6,.0f}  {str(r.position):<4}"
                      f"{str(r.team):<6}{str(r.name)[:30]}")
        else:
            print("  -> nothing expensive is missing; the misses are depth "
                  "chart padding, which is where a join is allowed to fail.")
