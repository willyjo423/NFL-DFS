...
def main(argv=None) -> int:
    ...
    if len(gradeable):
        ...
        for r in worst.itertuples(index=False):
            print(f"     ours {r.mean:>6.1f}  theirs {r.dk_points_per_game:>6.1f}"
                  f"   {str(r.name)[:24]:<26}{str(r.position):<4}{r.team}")
        if mae <= 1.0:
    ...
    if len(missing):
        ...
        if played.any():
            ...
            for r in missing[played].nlargest(12, "salary").itertuples(
                    index=False):
                print(f"     ${r.salary:>6,.0f}  {str(r.position):<4}"
                      f"{str(r.team):<6}{str(r.name)[:26]:<28}"
                      f"DK avg {r.dk_points_per_game:.1f}")
        else:
