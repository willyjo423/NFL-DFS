...
def prove_no_leak(df: pd.DataFrame, players: int = 150) -> bool:
    ...
    after = add_baselines(tampered.drop(
        columns=["prior_mean", "prior_last", "prior_position"]))
...
def walk(built: pd.DataFrame, test_weeks: list[tuple[int, int]],
         min_train: int = 2000) -> pd.DataFrame:
...
def main(argv=None) -> int:
    ...
    g = walk(built, test_weeks)
    print(f"\n  {len(g):,} graded player-weeks")
