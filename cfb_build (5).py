...
def main(argv=None) -> int:
    ...
    # Second pass: the players DraftKings says have scored but we did not
    # find. Name similarity proposes a candidate; DraftKings' own published
    # average decides whether to accept it. A wrong player fails that test,
    # because two different people do not score the same.
    gap_rows = merged[merged["games"].isna()
                      & (merged["dk_points_per_game"].fillna(0) > 0)]
    rescued, attempts = cfb.rescue_matches(gap_rows, hist)
    if rescued:
        for i, r in merged.iterrows():
            got = rescued.get((r["school"], r["name"]))
            if got:
                for col in ("games", "mean", "best", "floor",
                            "per_team_game", "team_games"):
                    merged.at[i, col] = got[col]
    ...
    # Graded ONLY on players who appear in every one of their team's games.
    # For anyone else the two sides are counting different denominators:
    # DraftKings divides by games it has the player ACTIVE for, and CFBD emits
    # no row at all for a player who recorded nothing, so our count runs short
    # for exactly the low-usage players. Grading across both groups measured
    # that bookkeeping difference and called it a scoring error - twice, in
    # opposite directions, 2.00x and then 0.50x.
    full = merged["games"] == merged["team_games"]
    gradeable = merged[has_hist & full
                       & merged["dk_points_per_game"].notna()].copy()
    partial = int((has_hist & ~full).sum())
    print(f"  {partial} players appear in fewer games than their team has")
    print(f"  played; they are excluded here because the two sides divide by")
    print(f"  different denominators, which is bookkeeping, not scoring.\n")
    if len(gradeable):
        ...
        gradeable["gap"] = gradeable["mean"] - gradeable["dk_points_per_game"]
        ...
        for r in worst.itertuples(index=False):
            ratio = ("-" if not r.dk_points_per_game
                     else f"{r.mean / r.dk_points_per_game:.2f}x")
            print(f"     ours {r.mean:>6.1f}  theirs "
        ...
        if len(phantom):
            ...
            for r in phantom.nlargest(6, "per_team_game").itertuples(
                ...
                print(f"     {str(r.name)[:24]:<26}{str(r.team):<6}"
                      f"ours {r.mean:.1f}   others named similarly "
    ...
    sub("NAMES RECOVERED BY MATCHING ON POINTS")
    if attempts:
        print("  Name similarity proposes; DraftKings' published average")
        print("  decides. A wrong player cannot reproduce the right score.\n")
        print(f"  {'draftkings':<24}{'college box score':<22}{'DK':>6}{'ours':>7}"
              f"{'gap':>7}  verdict")
        for dk_name, cand, team, target, got, how, gap, ok in attempts:
            print(f"  {str(dk_name)[:23]:<24}{str(cand)[:21]:<22}{target:>6.1f}"
                  f"{got:>7.1f}{gap:>7.2f}  {'ACCEPT' if ok else 'reject'}")
        print(f"\n  {len(rescued)} recovered, "
              f"{len(attempts) - len(rescued)} rejected and left unmatched.")
    else:
        print("  Nothing to recover: no unmatched player has DraftKings points.")
