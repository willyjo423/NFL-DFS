...
def attach_history(board: pd.DataFrame, hist: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    # Two averages, because they answer different questions and confusing
    # them cost a whole run. `mean` divides a player's points by the games HE
    # played; `per_team_game` divides by the games his TEAM played. A backup
    # who went off once has a huge `mean` and a modest `per_team_game`, and
    # only the second is comparable to DraftKings' published figure - which is
    # why our numbers came out at exactly 2.00x theirs for every player who
    # had appeared in one of his team's two games.
    team_games = (hist.groupby("school")["game_id"].nunique()
                  .rename("team_games"))
    agg = (hist.groupby(["school", "name"])
           ...
           .reset_index())
    agg = agg.join(team_games, on="school")
    agg["per_team_game"] = (agg["total"]
                            / agg["team_games"].clip(lower=1)).round(2)
    ...
    for r in board.itertuples(index=False):
        ...
        for i, key in enumerate(sorted(name_keys(r.name))):
            ...
            if found:
                ...
                break
        rows.append(found or (np.nan,) * 6)

    out = board.copy()
    out[["games", "mean", "best", "floor", "per_team_game",
         "team_games"]] = pd.DataFrame(rows, index=board.index)
...
def near_misses(name: str, school: str, hist: pd.DataFrame, n: int = 3):
    """Who in this school's history looks most like a player we failed to find.

    A join failure that says only "not found" makes the next run a repeat of
    this one. A join failure that prints the three closest names in the same
    school either shows the spelling difference at a glance - Matt against
    Matthew - or shows that nobody remotely similar exists, which means the
    player genuinely has no box score and the failure is DraftKings', not
    ours. Either way the next step is decided by reading, not by guessing.
    """
    import difflib
    pool = hist.loc[hist["school"] == school, "name"].dropna().unique().tolist()
    return difflib.get_close_matches(str(name), pool, n=n, cutoff=0.55)


def rescue_matches(unmatched: pd.DataFrame, hist: pd.DataFrame,
                   tolerance: float = 0.75) -> tuple[pd.DataFrame, list]:
    """Recover the names two sources spell differently, using points as proof.

    The fifteen failures the last run printed were all one thing: college
    rosters use given names and DraftKings uses what the player is called.
    Matthew/Matt, Benjamin/Benji, Maxence/Max, Michael/Mike, Quintrevion/Tre,
    Caleb/Champ. Plus a dropped hyphen (Sacovie White-Helton against Sacovie
    White), a middle initial (Tyler J. Williams), a space (JoJo/Jo Jo), and
    one outright typo in CFBD - Remenowksy for Remenowsky.

    No exact reduction reaches those. Fuzzy matching does, and fuzzy matching
    is exactly what I have refused to do all along, because an edit-distance
    match pairs two different players and is worse than a miss.

    What makes it safe here is that DraftKings publishes the player's average,
    so a proposed match can be CHECKED rather than trusted. The name proposes;
    the points dispose. A candidate is accepted only if his scoring in our
    data reproduces DraftKings' published figure under one of the two divisors
    - and a wrong player will not, because two different people do not score
    the same. Everything rejected stays unmatched and is reported.
    """
    import difflib
    if unmatched.empty or hist.empty:
        return unmatched.assign(), []

    by_school = {s: g for s, g in hist.groupby("school")}
    team_games = hist.groupby("school")["game_id"].nunique()

    accepted, log_rows = {}, []
    for r in unmatched.itertuples(index=False):
        target = r.dk_points_per_game
        if not target or not np.isfinite(target) or target <= 0:
            continue
        pool = by_school.get(r.school)
        if pool is None:
            continue
        names = pool["name"].dropna().unique().tolist()
        for cand in difflib.get_close_matches(str(r.name), names, n=4,
                                              cutoff=0.5):
            rows = pool[pool["name"] == cand]
            total = float(rows["dk_points"].sum())
            played = len(rows)
            tg = int(team_games.get(r.school, played) or played)
            options = {"per game played": total / max(played, 1),
                       "per team game": total / max(tg, 1)}
            best = min(options.items(), key=lambda kv: abs(kv[1] - target))
            gap = abs(best[1] - target)
            ok = gap <= max(tolerance, 0.05 * target)
            log_rows.append((r.name, cand, r.team, target, round(best[1], 2),
                             best[0], round(gap, 2), ok))
            if ok:
                accepted[(r.school, r.name)] = {
                    "games": played, "mean": round(total / max(played, 1), 2),
                    "best": float(rows["dk_points"].max()),
                    "floor": float(rows["dk_points"].min()),
                    "per_team_game": round(total / max(tg, 1), 2),
                    "team_games": tg, "matched_as": cand}
                break
    log.info("rescued %d of %d unmatched-but-scoring players by name "
             "similarity confirmed against DraftKings' own average",
             len(accepted), len(unmatched))
    return accepted, log_rows
