...
# Printed in the header on every run. Two runs in a row produced output that
# was identical byte for byte, including strings that had been deleted from
# this file - which meant the repo was still running the previous version and
# the fix was never under test. A script that does not say which version it is
# makes that indistinguishable from "the fix did not work", and the difference
# between those two costs a whole round trip to find out.
VERSION = "v4"
...
def _s(x) -> str:
    """A string, always. The last run died on `r.name[:26]` because the field
    was None, which turned a mapping problem into a traceback and threw away
    everything the run had left to print."""
...
def _normalise_board(raw: list[dict]) -> pd.DataFrame:
    """Two different DraftKings schemas, one frame.

    `api.draftkings.com/draftgroups` answers 403 from GitHub's runners - all
    three URL shapes, Access Denied in HTML - so the only working source is the
    lobby's own `getavailableplayers`, and it speaks a different, abbreviated
    dialect: `fn`/`ln` instead of displayName, `s` instead of salary, `pp`
    instead of position, and no team abbreviation at all.

    The team is the interesting one. That payload carries `tid` (the player's
    team id) alongside `htid`/`atid` and `htabbr`/`atabbr` for the fixture, so
    the abbreviation is recovered by asking which side of the game he is on.
    Guessing instead would attach players to the wrong opponent and the wrong
    implied total - numbers that look right in every row and are wrong in
    every row.
    """
    rows = []
    for p in raw:
        name = _pick(p, "displayName", "Name", "name")
        if not name:
            first, last = _pick(p, "fn", "firstName"), _pick(p, "ln", "lastName")
            name = " ".join(x for x in (first, last) if x) or None

        team = _pick(p, "teamAbbreviation", "tabbr", "ta")
        if not team:
            tid = _pick(p, "tid", "teamId")
            htid, atid = _pick(p, "htid"), _pick(p, "atid")
            if tid is not None and tid == htid:
                team = _pick(p, "htabbr")
            elif tid is not None and tid == atid:
                team = _pick(p, "atabbr")

        home, away = _pick(p, "htabbr"), _pick(p, "atabbr")
        game = f"{away} @ {home}" if home and away else None

        rows.append({
            "name": name,
            "position": _pick(p, "position", "pp", "pn", "posn"),
            "team": team,
            "salary": pd.to_numeric(_pick(p, "salary", "s"), errors="coerce"),
            "status": _pick(p, "status", "IsDisabledFromDrafting"),
            "game": game,
        })
    ...
    # Say what the mapping actually achieved. A mapper that silently produces a
    # frame of Nones is the same failure as the join that silently matches
    # nothing: it only looks like success until something downstream uses it.
    got = {c: int(df[c].notna().sum()) for c in
           ("name", "position", "team", "salary")}
    print(f"\n  mapped {len(df)} rows -> " +
          ", ".join(f"{k} {v}/{len(df)}" for k, v in got.items()))
    if got["name"] == 0 or got["salary"] == 0:
        return pd.DataFrame()
    for field, n in got.items():
        if n and n < len(df):
            print(f"     {len(df) - n} rows have no {field}")
...
def dk_board(dg: int):
    ...
    Several variants, because the football model's URL carries no query string
    and sends no Accept header while this one added both, and because
    DraftKings serves the same board from more than one path. Rather than
    reason about which difference matters, try them and print the answer.
    """
    ...
    variants = [
        ...
        ("draftgroups/v1 without the repeated segment",
         f"https://api.draftkings.com/draftgroups/v1/{dg}/draftables", plain),
        ("the lobby's own player endpoint",
         f"https://www.draftkings.com/lineup/getavailableplayers?"
         f"draftGroupId={dg}", plain),
    ]
    raw = []
    for label, url, hdr in variants:
        ...
        try:
            ...
            continue
        # The lobby endpoint calls the same thing `playerList`, and a payload
        # whose only useful key is one this code does not look for is
        # indistinguishable from an empty payload - so say which keys arrived.
        raw = (payload.get("draftables") or payload.get("playerList")
               or payload.get("players") or [])
        if not raw:
            print(f"     parsed as JSON but no player list. "
                  f"Top-level keys: {sorted(payload)[:12]}")
            continue
        print(f"     parsed: {len(raw)} player rows")
        break
    if not raw:
    ...
    # The whole schema, not the first twelve keys. Last run printed twelve
    # alphabetically, which was enough to prove the schema was different and
    # not enough to map it - so the next run had to be spent on the same
    # question. Printing all of it, plus one row verbatim, ends that loop.
    fields = sorted({k for p in raw[:50] for k in p})
    print(f"\n  every field on a player row ({len(fields)}):")
    print(f"     {fields}")
    print("\n  one row exactly as it arrived:")
    for k in fields:
        v = raw[0].get(k)
        if v not in (None, "", [], {}):
            print(f"     {k:<26} {str(v)[:56]}")

    df = _normalise_board(raw)
    if df.empty:
        print("\n  Could not find name, salary and position under any known")
        print("  spelling. The field list above is the answer - map it.")
        return df
    ...
    print(f"\n  {len(df)} distinct players, {df['team'].nunique()} teams, "
          f"{df['game'].nunique()} games")
    if df["salary"].notna().any():
        print(f"  salary ${df['salary'].min():,.0f} to "
              f"${df['salary'].max():,.0f}\n")
    print("  positions on the board:")
    for pos, n in df["position"].value_counts().items():
        s = df.loc[df["position"] == pos, "salary"].dropna()
        if not len(s):
            print(f"     {str(pos):<8}{n:>5}   (no salary parsed)")
            continue
    ...
    print("\n  the priciest players, as a sanity check on the mapping:")
    for r in df.nlargest(8, "salary").itertuples(index=False):
        sal = "-" if pd.isna(r.salary) else f"${r.salary:,.0f}"
        print(f"     {_s(r.name)[:26]:<28}{_s(r.position):<5}{_s(r.team):<6}"
              f"{sal:>9}   {_s(r.game)[:34]}")
    return df
...
def main(argv=None) -> int:
    ...
    if not key:
        ...
        return 1
    print(f"  script {VERSION}   key present ({len(key)} chars), "
          f"season {args.season}")
    print(f"  If the line above does not say v4, GitHub is running an older")
    print(f"  copy of this file and nothing below reflects the current fixes.")
