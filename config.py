...
# api.draftkings.com answers 403 Access Denied to GitHub's runners on every
# URL shape - an IP-level block, not a wrong path. The lobby's own player
# endpoint sits on www.draftkings.com, the same host as the contest list that
# has been working all along, and returns the whole board. Kept here so the
# old path can be retried if the block ever lifts.
